...
class cart_class {
    ...
    public function clear_cart($customer_id) {
        try {
            if (empty($customer_id)) {
                error_log("Clear cart failed: empty customer_id");
                return false;
            }
            ...
            if ($result) {
                ...
                error_log("Clear cart: deleted $rows_deleted rows for customer $customer_id");
                return true; // Return true even if 0 rows (cart was already empty)
            }

            error_log("Clear cart failed: execute returned false");
            return false;
        } catch (Exception $e) {
