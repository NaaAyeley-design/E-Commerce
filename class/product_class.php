...
class product_class extends db_class {
     ...
     *
     * @param int $producer_id Producer's customer ID
     * @param int $limit Optional limit
     * @param int $offset Optional offset
     * @return array Array of products
     */
    public function get_products_by_producer($producer_id, $limit = 50, $offset = 0) {
        $sql = "SELECT 
                    p.product_id,
                    p.product_title,
                    p.product_price,
        ...
        return $this->fetchAll($sql, [$producer_id, $limit, $offset]);
    }

    /**
     * Count total products for a producer
     *
     * @param int $producer_id Producer's customer ID
     * @return int Total count
     */
    public function count_producer_products($producer_id) {
        $sql = "SELECT COUNT(*) as total FROM products WHERE producer_id = ?";
        $result = $this->fetchRow($sql, [$producer_id]);
        return $result ? (int)$result['total'] : 0;
    }

    /**
     * Get producer product statistics
     *
     * @param int $producer_id Producer's customer ID
     * @return array Statistics including total, active, draft, out_of_stock counts
     */
    public function get_producer_stats($producer_id) {
        $sql = "SELECT 
                    ...
                    SUM(CASE WHEN stock_quantity <= low_stock_threshold AND track_inventory = 1 THEN 1 ELSE 0 END) as low_stock_products,
                    SUM(CASE WHEN stock_quantity = 0 AND track_inventory = 1 THEN 1 ELSE 0 END) as out_of_stock_products,
    ...
    /**
     * Get single product for producer (with ownership check)
     *
     * @param int $product_id Product ID
     * @param int $producer_id Producer's customer ID
     * @return array|false Product data or false if not found/not owned
     */
    public function get_producer_product($product_id, $producer_id) {
        $sql = "SELECT 
                    p.*,
                    c.cat_name,
                    b.brand_name
        ...
        return $this->fetchRow($sql, [$product_id, $producer_id]);
    }
     ...
     *
     * @param int $product_id Product ID
     * @param int $producer_id Producer ID (for security)
     * @param int $new_quantity New stock quantity
     * @return bool Success
     */
    public function update_product_stock($product_id, $producer_id, $new_quantity) {
        $sql = "UPDATE products 
                SET stock_quantity = ?, updated_at = NOW() 
                WHERE product_id = ? AND producer_id = ?";

        $stmt = $this->execute($sql, [$new_quantity, $product_id, $producer_id]);
     ...
     * @param int $product_id Product ID
     * @param int $producer_id Producer ID (for security)
     * @param string $status New status (draft, active, inactive, out_of_stock)
     * @return bool Success
     */
    public function update_product_status($product_id, $producer_id, $status) {
        ...
        $sql = "UPDATE products 
                SET product_status = ?, updated_at = NOW() 
                WHERE product_id = ? AND producer_id = ?";

        $stmt = $this->execute($sql, [$status, $product_id, $producer_id]);
        return $stmt && $stmt->rowCount() > 0;
    }
     ...
     */
    public function get_low_stock_products($producer_id) {
        $sql = "SELECT 
                    p.product_id,
                    p.product_title,
                    p.stock_quantity,
        ...
        return $this->fetchAll($sql, [$producer_id]);
    }

    /**
     * Search producer's products
     *
     * @param int $producer_id Producer's customer ID
     ...
     * @return array Array of matching products
     */
    public function search_producer_products($producer_id, $search_term, $limit = 50) {
        $sql = "SELECT 
                    p.product_id,
                    p.product_title,
                    p.product_price,
                ...
                LEFT JOIN brands b ON p.product_brand = b.brand_id
                WHERE p.producer_id = ? 
                AND (p.product_title LIKE ? OR p.product_desc LIKE ? OR p.sku LIKE ?)
                ORDER BY p.product_title ASC
                LIMIT ?";

        $search_param = '%' . $search_term . '%';
        return $this->fetchAll($sql, [$producer_id, $search_param, $search_param, $search_param, $limit]);
    }

    /**
     * Filter producer products by status
     *
     * @param int $producer_id Producer's customer ID
     ...
     * @return array Array of products
     */
    public function filter_producer_products_by_status($producer_id, $status, $limit = 50, $offset = 0) {
        $sql = "SELECT 
                    p.product_id,
                    p.product_title,
                    p.product_price,
                ...
                LEFT JOIN categories c ON p.product_cat = c.cat_id
                LEFT JOIN brands b ON p.product_brand = b.brand_id
                WHERE p.producer_id = ? AND p.product_status = ?
                ORDER BY p.created_at DESC
                LIMIT ? OFFSET ?";

        return $this->fetchAll($sql, [$producer_id, $status, $limit, $offset]);
    }

    /**
     * Delete a product (producer ownership check)
     *
     * @param int $product_id Product ID
     * @param int $producer_id Producer ID (for security)
     * @return bool Success
     */
    public function delete_producer_product($product_id, $producer_id) {
        $sql = "DELETE FROM products WHERE product_id = ? AND producer_id = ?";
        $stmt = $this->execute($sql, [$product_id, $producer_id]);
        return $stmt && $stmt->rowCount() > 0;
    }
