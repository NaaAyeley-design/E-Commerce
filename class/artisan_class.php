...
/**
 * Get all active artisans - FIXED VERSION
 * Replace lines 17-30 in artisan_class.php
 */
public function get_all_artisans($limit = 50, $offset = 0)
{
    try {
        ...
        if (!$conn) {
            error_log("get_all_artisans: No database connection");
            return [];
        }
        ...
        error_log("get_all_artisans: Found " . count($results) . " artisans");

        return $results ? $results : [];

    } catch (PDOException $e) {
        error_log("get_all_artisans PDO error: " . $e->getMessage());
        return [];
    } catch (Exception $e) {
        error_log("get_all_artisans error: " . $e->getMessage());
        return [];
    }
}
