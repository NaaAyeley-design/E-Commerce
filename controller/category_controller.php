...
function update_category_ctr($cat_id, $cat_name, $user_id) {
    try {
        ...
        // For admin users, allow updating any category
        // For regular users, check ownership
        if (is_admin()) {
            // Check if category exists (without user_id filter for admins)
            $existing_category = $category->get_category_by_id_admin($cat_id);
            if (!$existing_category) {
                return "Category not found.";
            }

            // For admins, check if name exists globally (excluding current category)
            if ($category->category_name_exists_global($cat_name, $cat_id)) {
                return "A category with this name already exists.";
            }

            // Update the category (admin override)
            $updated = $category->update_category_admin($cat_id, $cat_name);
        } else {
...
function delete_category_ctr($cat_id, $user_id) {
    try {
        ...
        // For admin users, allow deletion of any category
        // For regular users, check ownership
        if (is_admin()) {
            // Check if category exists (without user_id filter for admins)
            $existing_category = $category->get_category_by_id_admin($cat_id);
            ...
            // Delete the category (admin override)
            $deleted = $category->delete_category_admin($cat_id);
        } else {
            // Check if category exists and belongs to user
            $existing_category = $category->get_category_by_id($cat_id, $user_id);
            if (!$existing_category) {
